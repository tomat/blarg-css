    ...
    protected function seek($selector, &$ret) {
        ...
        for($i=$this->_[HDOM_INFO_BEGIN]+1; $i<$end; ++$i) {
            ...
            if ($pass && $key && $val  && $val!=='*') {
                ...
                // handle id and multiple class
                if (!$check && strcasecmp($key, 'id')===0) {
                    $blarg_vals = explode(".",$val);
                    if(count($blarg_vals) > 1)
                    {
                        $blarg_i = 0;
                        foreach($blarg_vals as $blarg_val)
                        {
                            if($blarg_i == 0)
                            {
                                $check = $this->match($exp, $blarg_val, $node->attr['id']);
                                if(!$check) break;
                            }
                            elseif(isset($node->attr['class']))
                            {
                                $check = true;
                                foreach(explode(' ',$node->attr['class']) as $k) {
                                    $check = $this->match($exp, $blarg_val, $k);
                                    if($check) break;
                                }
                                if(!$check) break;
                            }
